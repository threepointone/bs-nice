...
function string_of_scope(scope, hash, content) {
  ...
  if (List.length(scope[/* selectors */2]) > 0) {
    prefix = prefix + $$String.concat("", scope[/* selectors */2]);
  }
  prefix = prefix + "{";
  suffix = suffix + "}";
...
function group(normalized) {
  var result = List.fold_left((function (param, param$1) {
          var style = param$1[1];
          var scope = param$1[0];
          var styles = param[2];
          var lastScope = param[1];
          var rest = param[0];
          var match = Caml_obj.caml_equal(lastScope, scope);
          if (match !== 0) {
            return /* tuple */[
                    rest,
                    scope,
...
function css(decls) {
  var g = group(flatten(decls));
  var className = "css-" + Pervasives.string_of_int(Hashtbl.hash(g));
  var cssRules = List.map((function (param) {
          return string_of_scope(param[0], className, $$String.concat("; ", List.map(string_of_style, param[1])));
        }), g);
  console.log($$String.concat("\n", cssRules));
  Curry._1(insertRule, $$String.concat("\n", cssRules));
  return className;
}
