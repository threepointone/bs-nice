...
function replace(str, _with) {
  return _with.replace(replacementRegex, str);
}
...
function joinSelectors(selectors) {
  var joinSelectors$1 = function (selectors) {
    if (selectors) {
      ...
      if (t) {
        if (t[1]) {
          return joinSelectors$1(t).replace(replacementRegex, x);
        } else {
          return t[0].replace(replacementRegex, x);
        }
      } else {
        return x;
      }
...
function string_of_scope(scope, hash, content) {
  ...
  if (List.length(scope[/* selectors */2]) > 0) {
    prefix = prefix + hash.replace(replacementRegex, joinSelectors(scope[/* selectors */2]));
  }
  prefix = prefix + "{";
  suffix = suffix + "}";
